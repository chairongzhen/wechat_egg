...
class AccountSerive extends Service {
    ...
    async register(username,pwd,nickname) {
        ...
        if(existres[0].existcount > 0) { 
            ...
            let newid = UUID.v1();
            let insertSql = `INSERT INTO appuser (uid,username,pwd,nickname,openid,isdelete) values ("${newid}","${username}","${pwd}","${nickname}","${newid}",0) insert into users (openid,nickname,gender,province,city,country,headimgurl,createdate,lastonline) VALUES ("${newid}","${nickname}","男","上海","上海","",now(),now()) `;
